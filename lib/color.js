...
var applyAlpha = color.applyAlpha = function(vector, alpha) {
  return new Float32Array([vector[0], vector[1], vector[2], vector[3] * alpha]);
}
...
function parseHexColor(colorString) {
  ...
  if (hexLen === 7) {
    ...
    var r = parseInt(colorString.substr(1, 1), 16);
    var g = parseInt(colorString.substr(2, 1), 16);
    var b = parseInt(colorString.substr(3, 1), 16);
    return new Float32Array([(r << 4 | r) / 255.0, (g << 4 | g) / 255.0, (b << 4 | b) / 255.0, 1.0]);
  } else {
    return undefined;
  }
}

function parseRGBAColor(colorString) {
  colorString = colorString.substr(5).split(/ *, */);
  var r = parseInt(colorString[0]);
  var g = parseInt(colorString[1]);
  var b = parseInt(colorString[2]);
  var a = parseFloat(colorString[3]);
  return new Float32Array([r / 255.0, g / 255.0, b / 255.0, a]);
}
...
function namedColor(colorString) {
...
var parseColor = color.parseColor = function(colorString) {
