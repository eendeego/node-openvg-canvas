...
module.exports.createCanvasRenderingContext2D = function(canvas) {
  ...
  function setStrokeStyle(newStrokeStyle) {
    if ('string' === typeof newStrokeStyle) {
      strokeColor = color.toVGVector(color.parseColor(newStrokeStyle));
  ...
  function setFillStyle(newFillStyle) {
    ...
    if ('string' === typeof newFillStyle) {
      fillColor = color.toVGVector(color.parseColor(newFillStyle));
  ...
  function createGradient(type, gradient) {
    var result = {
      type: type,
      gradient: gradient,
      stops: [],
      addColorStop : function(stop, color) {
        result.stops.push({
          stop: stop,
          color: color.toVFVector(color.parseColor(color))
        });
      }
    }
    return result;
