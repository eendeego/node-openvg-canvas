...
module.exports.createCanvasRenderingContext2D = function (canvas) {
  ...
  result.restore = function () {
    ...
    result.shadowColor = serializedShadowColor = current.serializedShadowColor;
  ...
  function applyGradient(paint, gradient, paintMode) {
    ...
    stopArray = gradient.calculateStopArray(globalAlpha);
  ...
  result.fillRect = function (x, y, w, h) {
    ...
    if (checkShadowEffects()) dropShadow(paintRect);
    paintRect();
  };
  ...
  result.stroke = function (path) {
    ...
    if (checkShadowEffects()) dropShadow(strokePath);
    strokePath();
  };
