...
module.exports.createCanvasRenderingContext2D = function (canvas) {
  ...
  // dashed lines
  var lineDashPattern = new Float32Array(0);
  ...
  result.save = function () {
    var current = {
      ...
      serializedShadowColor: serializedShadowColor,
  ...
  result.restore = function () {
    ...
    result.setLineDash(current.lineDashPattern);
  ...
  function setStrokeStyle(newStrokeStyle) {
    ...
    if (newStrokeStyle instanceof Gradient) {
      strokeGradient = newStrokeStyle;
      applyStrokeStyle = strokeStyleGradient;
    } else if ('pattern' === newStrokeStyle.type) {
  ...
  function setFillStyle(newFillStyle) {
    ...
    if (newFillStyle instanceof Gradient) {
      fillGradient = newFillStyle;
      applyFillStyle = fillStyleGradient;
    } else if ('pattern' === newFillStyle.type) {
  ...
  result.createPattern = function (image, repetition) {
    ...
    var result = {
      type: 'pattern',
      image: image,
      tilingMode: repetition === 'repeat' ? vg.VGTilingMode.VG_TILE_REPEAT : vg.VGTilingMode.VG_TILE_FILL,
      repetition: repetition,
      transform: new m.SVGMatrix(),
  ...
  function setShadowColor(newShadowColor) {
    shadowColor = color.parseColor(newShadowColor);
  ...
  result.fillRect = function (x, y, w, h) {
    ...
    if (checkShadowEffects()) dropShadow(paint);
    paint();
  };
  ...
  result.fill = function (path) {
    ...
    if (checkShadowEffects()) dropShadow(fillPath);
    fillPath();
  };

  result.stroke = function (path) {
    if (checkShadowEffects(vg.VGPaintMode.VG_STROKE_PATH)) {

    }
    applyStrokeStyle();
    (path || currentPath).stroke();
  };
  ...
  result.setLineDash = function (segments) {
    if (segments === lineDashOffset) return; // Nothing to do, just move along

    var size = segments.length;
    if (size % 2 === 1) size += size;
    var pattern = new Float32Array(size);
    ...
    if (size !== segments.length)
      pattern.set(pattern.subarray(0, segments.length), segments.length);
    lineDashPattern = pattern;
    vg.setFV(vg.VGParamType.VG_STROKE_DASH_PATTERN, pattern);
  };
